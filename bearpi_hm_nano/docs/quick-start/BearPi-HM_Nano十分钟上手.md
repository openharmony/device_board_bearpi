...
4. 导入镜像到本地磁盘（选择一个磁盘空间大小≥ 10G的盘），点击`导入`，如下图所示。
...
6. 此时虚拟机进入登录界面，点击用户头像，如下图所示。

    ![](figures/虚拟机登录页面.png)

7. 输入密码：bearpi，然后点击`登录`，如下图所示。

    ![](figures/ubuntu登录.png)

8. 进入桌面后，点击桌面空白处`右键`，点击`打开终端(E)`，如下图所示。

    ![](figures/ubuntu打开终端.png)

9. 在终端中输入`ifconfig`，然后点击回车，除`lo`外，另外一个就是的网卡信息，记录获取到的IP地址，如下图所示。
...
11. 附加：如果连不上网络（如果主机网络需要拨号，如`校园网络`、`ADSL拨号`等）
    ...
    然后在网络适配器中，改成`NET 模式`，点击`确定`，如下图所示。
...
2. 输入连接信息，远程地址，并点击OK，如下图所示。

    ![](figures/MobaX地址输入.png)

3. 输入账号：`bearpi`，点击回车，如下图所示。

    ![](figures/Mobax_账号输入.png)

4. 输入密码：`bearpi`，注意，输入密码的时候屏幕不会显示，输完之后点击`回车`，如下图所示。
